...
/** @param {{ supabase: import('@supabase/supabase-js').SupabaseClient, sendTelegramMessage: (order: unknown, items: unknown[]) => Promise<unknown> }} deps */
function buildOrdersRouter({ supabase, sendTelegramMessage }) {
  ...
  router.post('/create-order', async (req, res) => {
    try {
      ...
      if (source === 'crm' || source === 'staff') {
        ...
        if (profError) {
          return res.status(500).json({ error: 'Error de perfil', detail: profError.message })
        }
        if (!profile) {
          return res.status(403).json({ error: 'Acceso denegado', detail: 'Perfil inexistente' })
        }
        if (!profile.active) {
          return res.status(403).json({ error: 'Acceso denegado', detail: 'Perfil inactivo' })
        }
      }
  ...
  router.post('/order', async (req, res) => {
    try {
      ...
      const { data: order, error } = await supabase
        .from('orders')
        .insert({ order_number, table_number, items, total, notes, source: 'qr' })
        .select()
        .single()
