...
// Una sola contraseña en pantalla; el email del usuario staff va en
// VITE_OPS_AUTH_EMAIL (Vercel + .env). signInWithPassword sigue emitiendo JWT normal.
const OPS_EMAIL = import.meta.env.VITE_OPS_AUTH_EMAIL?.trim()
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('')
    if (!OPS_EMAIL) {
      setError('Falta configurar VITE_OPS_AUTH_EMAIL.')
      return
    }
    ...
    try {
      ...
      if (signErr) {
        if (signErr.message === 'Invalid login credentials') {
    ...
    >
      <div className="w-full max-w-sm rounded border border-[rgba(201,162,39,0.25)] bg-[rgba(13,32,16,0.85)] p-8">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            <label htmlFor="ops-password" className="mb-1 block font-accent text-xs text-[rgba(245,240,232,0.75)]">
              Contraseña
            </label>
            <input
              id="ops-password"
